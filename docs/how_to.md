...
- **viewer** не требует torch и скрейпинг-либ — это гарантируется тестом `tests/test_viewer_imports.py`. Эндпоинт `/api/similarity/search` (текст-поиск) в этом профиле отвечает `503`; поиск соседей по точкам и остальные страницы работают. Чтобы фронт в такой сборке вообще не показывал поле текст-поиска, выстави `MYTHO_SERVER__TEXT_SEARCH=false` — флаг едет на фронт в ответе `/api/similarity/models`, и страница Similarity прячет поиск (оставляя разбор соседей по точкам). Определять профиль автоматически по установленным пакетам осознанно не стали: pip не хранит, какой extra выбран, так что флаг задаётся явно.
