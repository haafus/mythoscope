...
`settings.py` → `graphs.max_entities` (env `MYTHO_GRAPHS__MAX_ENTITIES`, по умолчанию 100; `None` = оставить всё) ограничивает каждый граф **N самыми часто упоминаемыми** сущностями (по числу упоминаний до дедупа); рёбра остаются только между оставленными. В логе пишется, сколько найдено всего и сколько оставлено по каждому типу.
...
Пересобрать графы **из уже извлечённого кэша**, не дёргая LLM (например, после изменения логики построения графов). Перезаписывает выходные файлы, книги с неполным кэшем пропускает; API-ключ не нужен:

```bash
mytho graphs --regraph
```
...
Основные файлы:
- `src/motifs/build_motifs.py` — оркестратор шага (идемпотентный, возобновляемый; `--reprocess` пересобирает из кэша без сети, `--force` — с перезагрузкой источников).
...
**Об ареалах (декодирование).** Ареальные индексы (`.19.21.29.`) — это глобальные номера макро-ареалов Березкина. Они декодируются в названия так: на детальной странице мотива жирные заголовки макро-ареалов идут в том же возрастающем порядке, что и числовой список ареалов; для «чистого» мотива (без скобочных/сравнительных записей `(Ср. …)`) индекс i выравнивается 1:1 с заголовком i. Голосование таких выравниваний по всему каталогу даёт глобальную легенду `индекс → макро-ареал` (`berezkin.parse_area_headers` + `build_area_legend`). Это покрывает ~94% реально используемых индексов (~65 макро-ареалов; порядок выходит географически связным: Африка → Европа → Азия → Океания → Сибирь → Сев. Америка → Мезоамерика → Юж. Америка). Чтобы шум не попадал в легенду, имя принимается только при поддержке ≥2 мотивов; несколько редких индексов остаются нерасшифрованными и показываются числом. Декодирование требует загрузки детальных страниц (`berezkin_details`); без неё ареалы остаются числами. Per-region легенда из `areas1.html` (список этносов с другой нумерацией) для этого **не** годится и не используется.
...
Cross-walk Berezkin↔TMI намеренно отсутствует: каталог не содержит систематических кодов Томпсона (нужен опубликованный конкорданс). Подробный разбор источников — в [motif-index-data-sources.md](motif-index-data-sources.md).
