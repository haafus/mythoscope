...
**Обогащение из mapsofmyths.com** (английские названия/определения, таксономия type/group, прямые Thompson-id, распределение по традициям) — отдельный шаг пайплайна `mytho motifs`, кэшируется в `outputs/motifs/raw/mapsofmyths/`, результат — `outputs/motifs/mapsofmyths_*.json` (не коммитятся). Требует HTTP basic-auth: `MYTHO_MOTIFS__MAPSOFMYTHS_USER` / `MYTHO_MOTIFS__MAPSOFMYTHS_PASS`; без кредов шаг пишет предупреждение и пропускается (каталог собирается без обогащения). Библиография Томпсона (folkmasa + курируемый список) — тоже шаг пайплайна, пишет `outputs/motifs/tmi_bibliography.json`. Сколько и каких сущностей дообогащено — видно в `mytho status`.
