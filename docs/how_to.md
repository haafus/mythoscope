...
`settings.py` → `graphs.max_entities` (env `MYTHO_GRAPHS__MAX_ENTITIES`, по умолчанию не задан = оставить всё) ограничивает каждый граф **N самыми часто упоминаемыми** сущностями (по числу упоминаний до дедупа); рёбра остаются только между оставленными. В логе пишется, сколько найдено всего и сколько оставлено по каждому типу.
