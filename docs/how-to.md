...
Что внутри:
- **по умолчанию** — продукты `outputs/`: corpus, embeddings (каталог ChromaDB), projections, graphs, motifs; плюс локальные `file:`-источники корпуса из `sources/` (оригиналы едут вместе с бандлом, чтобы пересборка на целевой машине могла их переингестить); **без** кэшей и логов.
...
Быстрый end-to-end прогон для проверки пайплайна — ограничить корпус N документами
(по умолчанию 2). Можно передать своё число `N`:

```bash
mytho build --sample        # дефолтный лимит (2 документа)
mytho build --sample 50     # переопределить: 50 документов (или -s 50)
```

`--sample N` кэпит **только корпус** (собирает не больше N документов); эмбеддинги и
графы едут через тот же driver и берут свой счётчик из выхода корпуса — отдельно их
ограничивать не нужно. Это троттл **пропускной способности**, не спецификации:
`desired()` не меняется, недостающие документы остаются `missing` (не orphan, ничего не
удаляется), а обычный `mytho build` потом просто доберёт их — без перекачки и без churn.
Композится со scope: `mytho build corpus embeddings graphs --sample 5` прогонит пять
документов через эти три стадии, не трогая мотивы.
