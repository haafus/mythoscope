...
Быстрый end-to-end прогон для проверки пайплайна (по умолчанию 2 элемента на стадию):

```bash
mytho build --sample        # дефолтный лимит (2)
mytho build --sample 50     # переопределить: 50 (или -s 50)
```

`--sample N` кэпит **каждую стадию** до N ключей за прогон. Троттл пропускной способности,
не спецификации: `desired()` не меняется, недостающие ключи остаются `missing` (не orphan,
ничего не удаляется), обычный `mytho build` потом доберёт. Одноключевые стадии (projections,
motifs) не задеты — срез множества из одного элемента до первых N≥1 возвращает его же.
Композится со scope: `mytho build corpus embeddings graphs --sample 5`.
