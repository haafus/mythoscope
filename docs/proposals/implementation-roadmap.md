...
>    frozen `_TYPE_PAGES` probe — needs a manual re-probe), and no orphan detection for de-linked-but-200
...
`status` cannot. **The runbook is two passes over one baseline: snapshot → build → assert → reset →
build → assert.**
