...
- [`tradition-review.md`](tradition-review.md) — same end-to-end lens on the primary `tradition` field: unlike colour/major it is an authored source field and the cross-cutting join key, so the emphasis shifts to its lack of a canonical identity and the fragile string-match join between `config/corpus.json` and `config/traditions.json`
