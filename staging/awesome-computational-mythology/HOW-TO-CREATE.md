# How to create & populate the `awesome-computational-mythology` repo

This folder is a **ready-to-push scaffold**. It is NOT the final repo — copy it out of
`mythoscope` into a new standalone repository under your **personal** GitHub account.
...
- Public. **Do not** initialize with a README/license/gitignore (this scaffold provides them).
- Create.

## Step 2 — Push the scaffold

From your machine, copy this folder out and push it:

```sh
# copy the scaffold to a fresh location outside the mythoscope repo
cp -r staging/awesome-computational-mythology ~/awesome-computational-mythology
cd ~/awesome-computational-mythology
...
(Or use the GitHub web UI: "uploading an existing file" and drag the files in — but git is
easier for the `.github/` folder.)
...
- **Social preview:** upload a simple 1280×640 image.
...
## Step 8 — Promote (see `mythoscope` `docs/proposals/go-to-market.md` §8a)
...
*This scaffold lives in the `mythoscope` repo only as a staging convenience; once the standalone
repo exists you can delete `staging/awesome-computational-mythology/` from `mythoscope`.*
