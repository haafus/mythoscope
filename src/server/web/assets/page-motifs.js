...
function treeRow(node, depth, { current = false } = {}) {
    const count = node.child_count ? ` · ${node.child_count}` : "";
    const badge = `<span class="motifs-item-badge">L${escapeHtml(String(node.level ?? ""))}${count}</span>`;
...
function renderTmiTree(d) {
    ...
    rows.push(treeRow({ id: d.id, name: d.name, level: d.level, child_count: (d.children || []).length }, depth, { current: true }));
